...
class ChangeProfilePictureUseCase @Inject constructor(
    ...
    operator fun invoke(token : String, profile_image: MultipartBody.Part) : Flow<Resource<String>> = flow {
        try{
            emit(Resource.Loading())
            val message = repository.changeProfilePicture(token, profile_image)
