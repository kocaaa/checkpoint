...
class GetLocationsKeywordUseCase @Inject constructor(
    private val repository : CheckpointRepository
) {

    operator fun invoke(token : String, keyword : String) : Flow<Resource<List<Location>>> = flow {
        try{
            emit(Resource.Loading())
            val locations = repository.searchLocation(token, keyword).map{
                it.toLocation()
            };
