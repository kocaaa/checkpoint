...
class SearchUserFollowersUseCase @Inject constructor(
    private val repository: CheckpointRepository
){
    operator fun invoke(token : String, userId: Long, username : String) : Flow<Resource<List<UserDetailedDTO>>> = flow {
        try{
            emit(Resource.Loading())
            val userList = repository.getFollowersByUsername(token, userId, username)
